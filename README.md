...
- 在线阅读器支持：
  ...
  - 书签标记（多书签管理）
  - 阅读进度记忆
- 同一账号多设备续读：翻页或增删书签后自动保存页码与书签，重新打开时恢复上次状态，详情页显示已读比例
- 多设备同步采用版本号乐观锁：保存请求携带打开时的版本号，其他设备已写入新版本时返回 409 冲突并提示，不会覆盖较新的页码和书签
- 未登录时进度仅保留在本机（localStorage）；页码超出总页数或保存失败时保留原记录，不影响试读、夜间模式和字体设置
