...
- 在线阅读器支持：
  ...
  - 阅读进度记忆
- 同一账号多设备阅读进度同步：
  - 翻页或添加/取消书签后自动保存页码与书签，重新打开时恢复到上次位置
  - 电子书详情页展示已读比例，并提供「继续阅读」入口
  - 保存请求携带打开时获取到的版本号（乐观锁），另一台设备已写入新版本时旧请求返回 `409 冲突`，不会覆盖较新的页码和书签
  - 未登录时进度仅保存在本设备，不写入账号记录；页码超出总页数或保存失败时保留云端原记录
